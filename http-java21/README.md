...
2. Clone the [`examples` repository](https://github.com/unikraft-cloud/examples) and `cd` into the `examples/http-java21` directory:

```bash
git clone https://github.com/unikraft-cloud/examples
cd examples/http-java21/
```
...
```bash
kraft cloud deploy -p 443:8080 -M 1Gi .
```
...
[●] Deployed successfully!
 │
 ├────────── name: http-java21-5xw9m
 ├────────── uuid: b2c3d4e5-f6a7-8901-bcde-f12345678901
 ├───────── state: starting
 ├──────── domain: https://gentle-wind-b2x9pkqm.fra.unikraft.app
 ├───────── image: http-java21@sha256:4f8a2c6e1d9b3f7a5c0e2b4d8f6a1c3e7b9d2f4a6c8e0b2d4f6a8c0e2b4d6f
 ├──────── memory: 1 GiB
 ├─────── service: gentle-wind-b2x9pkqm
 ├── private fqdn: http-java21-5xw9m.internal
 ├──── private ip: 172.16.3.5
 └────────── args: /usr/lib/jvm/java-21-openjdk-amd64/bin/java -classpath /usr/src/ SimpleHttpServer
```

In this case, the instance name is `http-java21-5xw9m` and the address is `https://gentle-wind-b2x9pkqm.fra.unikraft.app`.
They're different for each run.

Use `curl` to query the Unikraft Cloud instance of the Java HTTP server instance:
...
http-java21-5xw9m   gentle-wind-b2x9pkqm.fra.unik...  running  since 3mins  http-java21@sha256:4f8a2c6e1d9b3f...  1 GiB   1      /usr/lib/jvm/java-21-openjdk-amd64/bin/java -classpath /usr/src/ SimpleHttpServer  421.30 ms
```

When done, you can remove the instance:

```bash
kraft cloud instance remove http-java21-5xw9m
```
